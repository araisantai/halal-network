...
chaincodeInvoke() {
    ...
    peer chaincode invoke -o localhost:7050 \
        ...
        --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_PENGUSAHA_CA \
        --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_BPJH_CA   \
        -c '{"function": "CreateSertifikatHalal","Args":["{\"id\":\"1\",\"perusahaan\":\"icetea\",\"addedAt\":1600138309939,\"nama\":\"Anubil Pavan\", \"nib\":\"097283834\",\"produk\":\"Es Teh\",\"alamat\":\"Samping Bara\",\"status\":\"Pengajuan Sertifikat\"}"]}'
...
chaincodeQuery() {
    setGlobalsForPeer0Pengusaha
    # setGlobalsForPengusaha
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "GetSertifikatHalalById","Args":["1"]}'
}
